"""
TODO:
- Make DQN inside `models.rl` and inherit from here.
"""
...
def preprocess_obs(obs, env_name, device):
    if env_name.split('-')[0].lower() == 'minigrid':
        return torch.tensor(obs['image'],dtype=torch.float).unsqueeze(0).to(device)
    else: # ALE, etc.
...
class DQNAgent(RLBaseAgent):
    ...
    def reset(self):
        ...
        # TODO: hypothesis: can't be reset, (set reset_timesteps @ cfg to False)
    ...
    def learn(self, timesteps=None):
        ...
        for t in trange(self.max_timesteps, leave=False):
            state = preprocess_obs(obs, self.env_name, self.device)
            ...
            next_state = preprocess_obs(next_obs, self.cfg['env_name'], self.device)
